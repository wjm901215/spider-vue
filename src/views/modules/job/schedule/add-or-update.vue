...
<script>
import {saveOrUpdate} from '@/api/job/schedule'
  ...
  methods: {
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          info(id, 'sys/schedule').then(({data}) => {
            if (data && data.code === 200) {
              let {beanName, methodName, params, cronExpression, remark, status} = data.schedule
              this.dataForm.beanName = beanName
              this.dataForm.methodName = methodName
              this.dataForm.params = params
              this.dataForm.cronExpression = cronExpression
